...
version: alpha
name: CPAM Conecta
description: "CPAM Conecta's visual system, adapted from a bold Scandinavian-fintech design language — an unusually heavy near-black display sans (weight 900) paired with Inter for UI and body, a vivid lime-green primary accent, sage-tinted surface neutrals, and generously rounded white cards on a pale green-tinted canvas. The system reads as a calm, confident operations tool rather than a typical dark admin panel."
...
typography:
  display-mega:
    fontFamily: CPAM Sans, Inter, system-ui, -apple-system, sans-serif
    fontSize: 126px
    fontWeight: 900
    lineHeight: 107.1px
  display-xxl:
    fontFamily: CPAM Sans, Inter, system-ui, sans-serif
    fontSize: 96px
    fontWeight: 900
    lineHeight: 81.6px
  display-xl:
...
CPAM Conecta's identity wears one signature pairing: a vivid lime-green `{colors.primary}` (`#9fe870`) reserved for every primary action, set against a pale sage-tinted canvas `{colors.canvas-soft}` (`#e8ebe6`) and a near-black ink `{colors.ink}` (`#0e0f0c`) with a hint of warmth from an underlying olive cast. The system reads calmer and friendlier than a typical dark admin panel — generous whitespace, large rounded cards, and an unusually heavy display sans at weight 900 carrying section headlines.

Display typography is the second decisive voice. `CPAM Sans` (documented substitute: Inter at weight 900, see Typography below) carries section headings at weight 900. It pairs with Inter at weight 600 for sub-heads — the contrast between the heavy display weight and Inter's neutrality creates the system's hierarchy: weight 900 for the moment that matters, Inter for everything else.

Cards are universally pill-rounded — `{rounded.xl}` 24 px is the system's signature card radius. Buttons take the same 24 px pill-rectangle shape. Sharp corners never appear on interactive chrome; the visual softness is part of the friendly, confident voice.

**Key Characteristics:**
- A single lime-green CTA accent `{colors.primary}` (`#9fe870`) — the system's universal primary action color. No second accent.
- Two-face display typography — CPAM Sans (weight 900, section-heading scale) + Inter (weight 600, sub-head scale). The contrast is the typographic story.
- `{rounded.xl}` 24 px is the canonical card and button radius. Generous, friendly.
- Sage-tinted canvas `{colors.canvas-soft}` (`#e8ebe6`) is the default page surface; white `{colors.canvas}` is reserved for cards lifted within the sage ground.
...
- Status is carried by `badge-positive` / `badge-negative` pills, never by the green accent — green stays reserved for primary actions only.

## Colors

### Brand & Accent
- **CPAM Green** (`{colors.primary}` — `#9fe870`): The brand's universal CTA color. Every primary button, every "Send money" pill, the brand's logo accent.
- **CPAM Green Hover** (`{colors.primary-active}` — `#cdffad`): The lighter green for active state.
- **CPAM Green Neutral** (`{colors.primary-neutral}` — `#c5edab`): A mid-saturation green used as a neutral active fill.
- **CPAM Green Pale** (`{colors.primary-pale}` — `#e2f6d5`): The lightest green for soft surface tints / badge backgrounds.
...
1. **CPAM Sans** — proprietary geometric sans with an unusually heavy weight 900 used for all hero displays. The face is the brand's typographic signature. Always at weight 900, never lighter on the marketing surface.
...
- **CPAM Sans for the brand voice, Inter for utility.** Strict role separation.

### Note on Font Substitutes
CPAM Sans is a proprietary-style choice for this project. Open-source substitutes:
...
- Background `{colors.ink}`, text `{colors.primary}` (CPAM green!), padding `{spacing.xl}`, shape `{rounded.xl}`. Used for promotional moments.
...
- Background `{colors.canvas-soft}`, text `{colors.ink}`, padding `{spacing.3xl} {spacing.xl}`. Headline in `{typography.display-mega}` (CPAM Sans weight 900).

**`hero-band-dark`** — the polarity-flipped dark hero.
- Background `{colors.ink}`, text `{colors.primary}` (CPAM green headline on near-black!), same padding / scale.
...
### Do
- Reserve `{colors.primary}` CPAM green for every primary CTA. The lime-green pill IS the brand's conversion signature.
- Set section headlines in `{typography.display-md}` / `{typography.display-xs}` CPAM Sans weight 900. Never lighter.
...
- Use the full semantic palette (positive / warning / negative) for in-product status — never repurpose CPAM green as a success indicator since it IS the primary-action color.

### Don't
- Don't introduce a second brand accent. CPAM green is the sole identity colour.
...
- Don't pair the green CTA with a green background. The system always sits CPAM green on neutral surfaces (sage / white / ink).
- Don't replace CPAM Sans with a generic geometric sans for section headlines — the heavy weight IS the system's voice.
